...
class OSRS(commands.Cog):
    ...
    async def link_osrs_name(self, ctx: discord.ApplicationContext, osrs_name: str):
        """Links a user's Discord ID to their OSRS username."""
        await ctx.defer(ephemeral=True)
        
        if not re.match(r"^[a-zA-Z0-9 _-]{1,12}$", osrs_name):
            return await ctx.followup.send("Invalid OSRS username format.", ephemeral=True)
        ...
        try:
            ...
            await ctx.followup.send(f"Your account has been linked to OSRS name: **{osrs_name}**.", ephemeral=True)
            ...
            await ctx.followup.send("An error occurred while linking your account.", ephemeral=True)

    @osrs_group.command(name="profile", description="View a member's OSRS stats from the Hiscores.")
    async def view_osrs_profile(self, ctx: discord.ApplicationContext, member: discord.Member = None):
        ...
        if not osrs_name:
            return await ctx.followup.send(f"{'You have' if target_member == ctx.author else f'{target_member.display_name} has'} not linked an OSRS name yet. Use `/osrs link`.", ephemeral=True)

        try:
            async with self.session.get(f"{self.hiscores_url}?player={up.quote(osrs_name)}") as response:
                if response.status == 404:
                    return await ctx.followup.send(f"OSRS name **{osrs_name}** not found on the Hiscores.", ephemeral=True)
                response.raise_for_status()
                data = await response.text()
        except aiohttp.ClientError as e:
            logger.error(f"Hiscores fetch failed for {osrs_name}: {e}")
            return await ctx.followup.send(f"Error fetching Hiscores data for **{osrs_name}**.", ephemeral=True)

        skills_data, _ = osrs_utils.parse_hiscores_data(data)
        
        if not skills_data:
            return await ctx.followup.send(f"Could not parse any skill data for **{osrs_name}**.", ephemeral=True)
    ...
    async def view_osrs_kc(self, ctx: discord.ApplicationContext, member: discord.Member = None):
