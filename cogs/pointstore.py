...
class PointStore(commands.Cog):
    ...
    admin_group = SlashCommandGroup("admin", "Admin commands for the point store.", default_permissions=discord.Permissions(manage_guild=True), parent=store_group)
    ...
    async def view_rewards(self, ctx: discord.ApplicationContext):
        ...
        try:
            ...
            await ctx.followup.send("An error occurred while fetching rewards.", ephemeral=True)
    ...
    async def redeem_reward(self, ctx: discord.ApplicationContext, reward_name: str):
        ...
        try:
            async with self.bot.db_pool.acquire() as conn, conn.transaction():
                ...
                if not reward:
                    return await ctx.followup.send(f"Reward '{reward_name}' not found or is currently inactive.", ephemeral=True)
                ...
                if user_points < reward['point_cost']:
                    return await ctx.followup.send(f"You need {reward['point_cost']:,} points, but you only have {user_points:,}.", ephemeral=True)
            ...
            await ctx.followup.send("An error occurred during the redemption process.", ephemeral=True)
    ...
    async def add_reward(self, ctx: discord.ApplicationContext,
        ...
        try:
            ...
            await ctx.followup.send(f"Reward '{name}' added to the store.", ephemeral=True)
            logger.info(f"Admin {ctx.author} added new reward: {name}")
        except asyncpg.exceptions.UniqueViolationError:
            await ctx.followup.send(f"A reward named '{name}' already exists.", ephemeral=True)
        except Exception as e:
            logger.error(f"Error adding new reward '{name}': {e}", exc_info=True)
            await ctx.followup.send("An error occurred while adding the reward.", ephemeral=True)
    ...
    async def remove_reward(self, ctx: discord.ApplicationContext, reward_name: str):
        ...
        async with self.bot.db_pool.acquire() as conn:
            ...
            if 'DELETE 1' in result:
                await ctx.followup.send(f"Reward '{reward_name}' has been permanently removed.", ephemeral=True)
                logger.info(f"Admin {ctx.author} removed reward: {reward_name}")
            else:
                await ctx.followup.send(f"Reward '{reward_name}' not found.", ephemeral=True)
    ...
    async def toggle_reward(self, ctx: discord.ApplicationContext, reward_name: str, is_active: bool):
        ...
        async with self.bot.db_pool.acquire() as conn:
            ...
            if 'UPDATE 1' in result:
                ...
                await ctx.followup.send(f"Reward '{reward_name}' has been {status}.", ephemeral=True)
