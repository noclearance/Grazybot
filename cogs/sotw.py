...
from config import WOM_CLAN_ID, WOM_SKILLS, SOTW_CHANNEL_ID
from helpers.ai import generate_announcement_json
from helpers.wom import create_competition, get_competition_details
from helpers.embeds import create_competition_embed
from helpers.utils import send_global_announcement
...
class SOTW(commands.Cog):
    ...
    async def start_sotw_logic(self, ctx_or_interaction, skill: str, duration_days: int):
        ...
        # Persist competition so /sotw view and background tasks can track it
        try:
            comp = data.get("competition") or data
            starts_at = datetime.fromisoformat(comp["startsAt"].replace("Z", "+00:00"))
            ends_at = datetime.fromisoformat(comp["endsAt"].replace("Z", "+00:00"))
            async with self.bot.db_pool.acquire() as conn:
                await conn.execute(
                    "INSERT INTO active_competitions (id, title, starts_at, ends_at) "
            ...
            await ctx_or_interaction.followup.send(
                f"SOTW created on WOM but failed to save locally: {e}", ephemeral=True
            )
            return
        ...
        if sotw_channel:
            ...
            try:
                from helpers.hub import push_to_hub
                await push_to_hub(
                    "sotw.started",
                    {
                        "id": comp["id"],
