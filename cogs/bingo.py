...
class Bingo(commands.Cog):
    ...
    async def start_bingo(self, ctx: discord.ApplicationContext, 
                          duration_days: Option(int, "How many days the event will last.", min_value=1, max_value=30)):
    ...
    async def complete_task(self, ctx: discord.ApplicationContext, 
                            task: Option(str, "The name of the task you completed."),
                            proof: Option(str, "A URL link to a screenshot or video proof.")):
    ...
    async def view_board(self, ctx: discord.ApplicationContext):
        ...
        if not event or not event['message_id']:
            return await ctx.respond("There is no active bingo board to display.", ephemeral=True)
        ...
        if channel:
            try:
                ...
                await ctx.respond("Could not find the original bingo board message.", ephemeral=True)
        else:
            await ctx.respond("Bingo channel not found.", ephemeral=True)
