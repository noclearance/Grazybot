...
class Bingo(commands.Cog):
    ...
    bingo_group = SlashCommandGroup("bingo", "Commands for clan bingo events.")
    ...
    async def start_bingo(self, ctx: discord.ApplicationContext,
        ...
        try:
            ...
            return await ctx.followup.send(f"Error: Could not load the bingo tasks file.", ephemeral=True)
        ...
        for difficulty, count in board_composition.items():
            if len(tasks_by_difficulty.get(difficulty, [])) < count:
                return await ctx.followup.send(f"Error: Not enough '{difficulty}' tasks in `{TASKS_FILE}`.", ephemeral=True)
        ...
        if not bingo_channel:
            return await ctx.followup.send("Error: Bingo Channel ID not configured correctly.", ephemeral=True)
        ...
        await ctx.followup.send(f"Bingo event created successfully in {bingo_channel.mention}!", ephemeral=True)
    ...
    async def complete_task(self, ctx: discord.ApplicationContext,
        ...
        async with self.bot.db_pool.acquire() as conn:
            ...
            if not event:
                return await ctx.followup.send("There is no active bingo event.", ephemeral=True)
            
            task_names = [t['name'] for t in json.loads(event['board_json'])]
            if task not in task_names:
                return await ctx.followup.send("That task is not on the current bingo board.", ephemeral=True)
            
            submission_id = await conn.fetchval(
                "INSERT INTO bingo_submissions (event_id, user_id, task_name, proof_url) VALUES ($1, $2, $3, $4) RETURNING id",
                event['id'], ctx.author.id, task, proof
            )

        admin_embed = discord.Embed(title="New Bingo Submission", description=f"**Task:** {task}", color=discord.Color.yellow())
        admin_embed.set_author(name=ctx.author.display_name, icon_url=ctx.author.display_avatar.url)
        ...
        await ctx.followup.send("Your submission has been sent for review!", ephemeral=True)
    ...
    async def view_board(self, ctx: discord.ApplicationContext):
        ...
        if not event or not event['message_id']:
            return await ctx.followup.send("There is no active bingo board to display.", ephemeral=True)
            
        channel = self.bot.get_channel(event['channel_id'] or config.BINGO_CHANNEL_ID)
        if channel:
            try:
                message = await channel.fetch_message(event['message_id'])
                await ctx.followup.send(f"Here is the current bingo board: {message.jump_url}")
            except discord.NotFound:
                await ctx.followup.send("Could not find the original bingo board message.", ephemeral=True)
        else:
            await ctx.followup.send("Bingo channel not found.", ephemeral=True)
