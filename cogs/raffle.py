...
class Raffle(commands.Cog):
    ...
    admin_group = SlashCommandGroup("admin", "Admin commands for raffles.", parent=raffle_group, default_permissions=discord.Permissions(manage_events=True))
    ...
    async def start_raffle(self, ctx: discord.ApplicationContext,
        ...
        if not raffle_channel:
            return await ctx.followup.send("Error: Raffle Channel ID not configured.", ephemeral=True)
        ...
        try:
            ...
            await ctx.followup.send(f"Raffle (ID: {raffle_id}) for **{prize}** created in {raffle_channel.mention}!", ephemeral=True)
            ...
            await ctx.followup.send("An error occurred while starting the raffle.", ephemeral=True)
        ...
        try:
            async with self.bot.db_pool.acquire() as conn:
                ...
                if not active_raffle:
                    return await ctx.followup.send("There is no active raffle to enter.", ephemeral=True)
                ...
                if self_entries >= 10:
                    return await ctx.followup.send(f"You have already claimed your max of 10 tickets for the '{prize}' raffle.", ephemeral=True)
            ...
            await ctx.followup.send(f"You have entered the **{prize}** raffle! You now have {total_tickets} ticket(s).", ephemeral=True)
        except Exception as e:
    ...
    async def give_tickets(self, ctx: discord.ApplicationContext,
        ...
        async with self.bot.db_pool.acquire() as conn:
            ...
            if not active_raffle:
                return await ctx.followup.send("There is no active raffle.", ephemeral=True)
        ...
        await ctx.followup.send(f"Gave {amount} ticket(s) to {member.display_name}. They now have {total_tickets} ticket(s).", ephemeral=True)
