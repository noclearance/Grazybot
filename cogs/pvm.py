...
class PVM(commands.Cog):
    ...
    async def schedule_pvm_event(self, ctx: discord.ApplicationContext,
        ...
        try:
            ...
            return await ctx.followup.send("Invalid start time format. Use 'YYYY-MM-DD HH:MM UTC'.", ephemeral=True)

        if event_start_dt <= datetime.now(timezone.utc):
            return await ctx.followup.send("Event start time must be in the future.", ephemeral=True)

        pvm_channel = self.bot.get_channel(config.PVM_EVENT_CHANNEL_ID)
        if not pvm_channel:
            return await ctx.followup.send("PVM Event Channel ID not configured.", ephemeral=True)
        ...
        try:
            ...
            await ctx.followup.send(f"PVM event '{title}' scheduled in {pvm_channel.mention}!", ephemeral=True)
            ...
            await ctx.followup.send("An error occurred while scheduling the event.", ephemeral=True)

    @pvm_group.command(name="participants", description="View participants for a PVM event.")
    async def view_pvm_participants(self, ctx: discord.ApplicationContext, event_id: int):
        ...
        try:
            async with self.bot.db_pool.acquire() as conn:
                ...
                if not event_data:
                    return await ctx.followup.send(f"PVM event with ID `{event_id}` not found.", ephemeral=True)
            ...
            await ctx.followup.send("An error occurred while fetching participants.", ephemeral=True)

    @pvm_group.command(name="cancel", description="Cancel an upcoming PVM event.")
    @commands.has_permissions(manage_events=True)
    async def cancel_pvm_event(self, ctx: discord.ApplicationContext, event_id: int):
        ...
        try:
            async with self.bot.db_pool.acquire() as conn:
                ...
                if not event_data:
                    return await ctx.followup.send(f"PVM event with ID `{event_id}` not found or already inactive.", ephemeral=True)
            ...
            await ctx.followup.send(f"PVM event '{event_data['title']}' (ID: {event_id}) successfully cancelled.", ephemeral=True)
            ...
            await ctx.followup.send("An error occurred while cancelling the event.", ephemeral=True)
