...
class Giveaway(commands.Cog):
    ...
    async def start_giveaway(self, ctx: discord.ApplicationContext,
        ...
        if delta is None:
            return await ctx.followup.send("Invalid duration format. Use 'd' for days, 'h' for hours, 'm' for minutes.", ephemeral=True)
        ...
        if not giveaway_channel:
            return await ctx.followup.send("Giveaway channel not found. Please configure it.", ephemeral=True)
        ...
        try:
            ...
            await ctx.followup.send(f"Giveaway for **{prize}** has been started in {giveaway_channel.mention}!", ephemeral=True)
            logger.info(f"Giveaway started by {ctx.author}: {prize}")
        except Exception as e:
            logger.error(f"Failed to start giveaway: {e}", exc_info=True)
            await ctx.followup.send("An error occurred while starting the giveaway.", ephemeral=True)

    @giveaway_group.command(name="entries", description="View the list of entrants for a giveaway.")
    @commands.has_permissions(manage_events=True)
    async def view_entries(self, ctx: discord.ApplicationContext, message_id: str):
        """Displays the list of users who have entered a giveaway."""
        await ctx.defer(ephemeral=True)
        try:
            msg_id = int(message_id)
        except ValueError:
            return await ctx.followup.send("Invalid message ID format.", ephemeral=True)

        try:
            async with self.bot.db_pool.acquire() as conn:
                giveaway_data = await conn.fetchrow("SELECT * FROM giveaways WHERE message_id = $1", msg_id)
                if not giveaway_data:
                    return await ctx.followup.send("No giveaway found with that message ID.", ephemeral=True)
            ...
            await ctx.followup.send("An error occurred while fetching entries.", ephemeral=True)
