...
class PersonalBests(commands.Cog):
    ...
    async def log_pb(self, ctx: discord.ApplicationContext,
        ...
        if not proof_url.startswith(('http://', 'https://')):
            return await ctx.followup.send("Proof URL must be a valid web link.", ephemeral=True)
        ...
        try:
            async with self.bot.db_pool.acquire() as conn:
                ...
                if existing_pb_ms and pb_time_ms >= existing_pb_ms:
                    return await ctx.followup.send(f"Your time ({time_in_seconds:.2f}s) is not faster than your current PB of {existing_pb_ms/1000:.2f}s.", ephemeral=True)
            ...
            await ctx.followup.send(f"Your PB for **{boss_name.title()}** has been updated to **{time_in_seconds:.2f}s**!", ephemeral=True)
            ...
            await ctx.followup.send("An error occurred while logging your PB.", ephemeral=True)

    @pb_group.command(name="my", description="View your Personal Best for a specific boss.")
    async def my_pb(self, ctx: discord.ApplicationContext, boss_name: str):
        ...
        try:
            ...
            if not pb_data:
                return await ctx.followup.send(f"You have no logged PB for **{boss_name.title()}**. Use `/pb log` to add one.", ephemeral=True)
            ...
            await ctx.followup.send("An error occurred while fetching your PB.", ephemeral=True)

    @pb_group.command(name="clan", description="View the clan leaderboard for a specific boss PB.")
    async def clan_pb(self, ctx: discord.ApplicationContext, boss_name: str):
        ...
        try:
            ...
            await ctx.followup.send("An error occurred while fetching the leaderboard.", ephemeral=True)
