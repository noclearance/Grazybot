...
import discord
import random
from config import RAFFLE_CHANNEL_ID, ANNOUNCEMENTS_CHANNEL_ID
...
async def draw_raffle_winner(bot, raffle_id: int) -> str:
    ...
    async with bot.db_pool.acquire() as conn:
        ...
        entries = await conn.fetch(
            "SELECT user_id FROM raffle_entries WHERE raffle_id = $1", raffle_id
        )

        if not entries:
            await raffle_channel.send(
                f"The raffle for **{prize}** has ended, but no one entered."
            )
            await conn.execute(
                "UPDATE raffles SET winner_id = 0 WHERE id = $1", raffle_id
            )
        ...
        if ann_channel:
            ann_embed = discord.Embed(
                ...
                description=(
                    f"{winner_user.mention} has won the **{prize}** raffle "
                    f"and **50 Clan Points**!"
                ),
                color=discord.Color.gold(),
            )
            await ann_channel.send(
                content=f"@everyone Congratulations to {winner_user.mention}!",
                embed=ann_embed,
            )

        await conn.execute(
            "UPDATE raffles SET winner_id = $1 WHERE id = $2", winner_id, raffle_id
        )
        ...
        try:
            ...
            await push_to_hub(
                "raffle.drawn",
                {"raffle_id": raffle_id, "winner_id": winner_id, "prize": prize},
            )
        except Exception:
            pass
