...
# NEW: Add a channel for giveaways, or use an existing one like announcements
...
class GiveawayView(discord.ui.View):
    ...
    async def enter_button(self, button: discord.ui.Button, interaction: discord.Interaction):
        """Allows a user to enter the giveaway."""
        conn = get_db_connection()
        cursor = conn.cursor()
        try:
            # Insert the user's entry, ignoring duplicates.
            cursor.execute(
            ...
            conn.commit()
            # cursor.rowcount will be 1 if a row was inserted, 0 if it was a duplicate.
...
async def generate_announcement_json(event_type: str, details: dict = None) -> dict:
    ...
    Your tone is epic, engaging, and slightly cheeky. You are here to build excitement and rally the members.
    ...
    Make every announcement sound like a legendary event is unfolding.
    """

    if event_type == "sotw_poll":
        specific_prompt = "Generate an embed for a new Skill of the Week poll. The description must implore the clan to lend their voice to the council, explaining that their choice will shape the clan's focus for the coming week. Frame it as a call to arms."
        fallback = {"title": "📊 A Council of Skills is Convened!", "description": "The time has come to choose our next battleground! Lend your voice to the council and cast your vote below, for your decision will shape our destiny this week.", "color": 15105600}
    elif event_type == "sotw_start":
        skill = details.get('skill', 'a new skill')
        specific_prompt = f"Generate an embed announcing the start of a grand Skill of the Week competition for the skill: **{skill}**. Describe it as a trial of dedication. Encourage warriors to hone their craft and declare that the gods of skill are watching. Mention that glory awaits the victor."
        fallback = {"title": f"⚔️ The Trial of {skill} Begins! ⚔️", "description": f"The clan has spoken! A grand trial of **{skill}** commences now. May the most dedicated warrior rise above all others and claim their rightful glory!", "color": 5763719}
    elif event_type == "raffle_start":
        prize = details.get('prize', 'a grand prize')
        specific_prompt = f"Generate an embed for a new clan raffle. The grand prize is **{prize}**. Speak of it as a gift from the gods of fortune. Clearly instruct members on how to enter by using the `/raffle enter` command to claim their ticket to destiny."
        fallback = {"title": "🎟️ Fortune's Favor is Upon Us!", "description": f"The gods of chance have smiled upon us! A new raffle has begun for a chance to win the legendary prize of **{prize}**. Use the `/raffle enter` command to claim your ticket and test your fate!", "color": 15844367}
    # NEW: Added giveaway event type
        ...
        specific_prompt = "Generate an embed announcing the start of a new clan bingo event. Describe it as a complex tapestry of trials woven by the Taskmaster himself. Challenge the clan to prove their versatility and teamwork by completing the tasks on the ancient board."
        fallback = {"title": "🧩 The Taskmaster's Gauntlet is Thrown! 🧩", "description": "Behold, a new trial of wit and skill! The Taskmaster has devised a fresh board of challenges to test your mettle. Let the games begin, and may the most versatile among you triumph!", "color": 11027200}
    elif event_type == "points_award":
        amount = details.get('amount', 'a number of')
        reason = details.get('reason', 'your excellent performance')
        specific_prompt = f"Generate an embed for a private message. The member has been awarded **{amount} Clan Points** for *{reason}*. Explain that these points are a measure of their renown and can be traded for legendary artifacts and boons in the clan's point store."
        fallback = {"title": "🏆 Your Renown Grows!", "description": f"For your dedication in *{reason}*, you have been awarded **{amount} Clan Points**! This currency of honor can be exchanged for great rewards. Well done.", "color": 5763719}
    else:
...
# --- UPDATED RAFFLE WINNER ANNOUNCEMENT ---
async def draw_raffle_winner(raffle_channel: discord.TextChannel):
    ...
    if not entries:
        ...
        # Mark raffle as complete even with no winner to prevent re-drawing
        ...
        if announcements_channel:
            ...
            # Send the announcement, tagging everyone and the winner in the content for a notification
...
# --- NEW: GIVEAWAY HELPER FUNCTIONS ---
async def end_giveaway(giveaway_data):
    """Handles the logic for ending a giveaway, drawing winners, and announcing them."""
    ...
    # Deactivate the giveaway first to prevent race conditions
    ...
    if not channel:
        print(f"Error: Could not find giveaway channel with ID {channel_id}")
        cursor.close(); conn.close()
        return

    # Try to fetch the original message to edit it
    try:
        message = await channel.fetch_message(message_id)
    except discord.NotFound:
        message = None
        print(f"Could not find original giveaway message {message_id}")
    ...
    if not user_ids:
        ...
        if message: await message.edit(view=None) # Disable the button
        cursor.close(); conn.close()
        return
    ...
    announcement_embed.set_footer(text="Thanks to all who participated!")

    await channel.send(content=f"Congratulations {', '.join(winner_mentions)}!", embed=announcement_embed)

    # Edit original message to show it has ended
    if message:
        ...
        await message.edit(embed=ended_embed, view=None) # Remove button
...
def generate_bingo_image(tasks: list, completed_tasks: list = []):
    try:
        ...
        # Use the basic, built-in font to avoid deployment issues.
        ...
        for i, task in enumerate(tasks):
            ...
            task_name = task['name']; wrapped_text = textwrap.fill(task_name, width=25) # Adjust width for default font
...
# --- Event Manager & Daily Reminder Tasks ---

# --- DAILY EVENT REMINDER TASK ---
@tasks.loop(hours=24)
async def daily_event_reminder():
    await bot.wait_until_ready()

    # Wait until a specific time of day to post, e.g., 14:00 UTC
    ...
    if not announcements_channel:
        print("Cannot send daily reminder: Announcements channel not found.")
        return
    ...
    cursor.execute("SELECT * FROM bingo_events WHERE ends_at > NOW() LIMIT 1")
    ...
    event_summary = ""
    if sotw:
        event_summary += f"- A Skill of the Week competition for **{sotw['title']}** is underway. Warriors can prove their might and view the leaderboard with `/sotw view`.\n"
    if raffle:
        event_summary += f"- A raffle for the legendary **{raffle['prize']}** is active. Members can seek fortune's favor by using `/raffle enter`.\n"
    if bingo:
        event_summary += "- The Taskmaster's Gauntlet (Bingo) challenges the clan's versatility. View the board with `/bingo board` and submit triumphs with `/bingo complete`.\n"

    if not event_summary:
        print("No active events for daily reminder.")
        return

    # Generate the announcement with the LLM
    prompt = f"""
    You are TaskmasterGPT, the wise and ancient lore-keeper for a clan of warriors.
    Your task is to write a daily bulletin summarizing the clan's active events. Your tone is epic, grand, and encouraging.
    Use the following information to compose your message. Frame it as a call to continue the good fight and remind everyone how to participate.
    ...
    try:
        ...
        embed = discord.Embed(title="📜 The Taskmaster's Daily Bulletin 📜", description=description, color=discord.Color.dark_gold())
        embed.set_footer(text="Seize the day, warriors!")
        await announcements_channel.send(embed=embed)
    except Exception as e:
        print(f"Failed to generate or send daily reminder: {e}")
...
async def event_manager():
    ...
    if raffle_channel:
        conn = get_db_connection(); cursor = conn.cursor(cursor_factory=psycopg2.extras.DictCursor)
        # Only look for raffles that have ended AND do NOT have a winner yet.
...
async def start_giveaway(
    ctx: discord.ApplicationContext,
    prize: discord.Option(str, "What is the prize for the giveaway?"),
    duration: discord.Option(str, "How long should it last? (e.g., 7d, 12h, 30m)"),
    winners: discord.Option(int, "How many winners should be chosen?", min_value=1, default=1)
):
    await ctx.defer(ephemeral=True)

    delta = parse_duration(duration)
    if delta is None:
        return await ctx.respond("Invalid duration format. Use 'd' for days, 'h' for hours, 'm' for minutes (e.g., `7d`, `12h`, `30m`).", ephemeral=True)

    ends_at = datetime.now(timezone.utc) + delta
    giveaway_channel = bot.get_channel(GIVEAWAY_CHANNEL_ID)

    if not giveaway_channel:
        return await ctx.respond(f"Error: Giveaway channel not found. Please check the `GIVEAWAY_CHANNEL_ID`.", ephemeral=True)
    ...
    try:
        # We need the message ID to create the view and save to DB
        # So we send the message first
        ...
        cursor = conn.cursor()
        cursor.execute(
            "INSERT INTO giveaways (message_id, channel_id, prize, ends_at, winner_count) VALUES (%s, %s, %s, %s, %s)",
            (giveaway_message.id, giveaway_channel.id, prize, ends_at, winners)
        )
        conn.commit()
        cursor.close()
        conn.close()
        ...
        await ctx.respond("I don't have permission to send messages or add views in the giveaway channel.", ephemeral=True)
    except Exception as e:
...
async def add_reward(
    ...
    name: str,
    cost: int,
    description: Option(str, "Optional description for the reward.", required=False),
    role_id: Option(str, "Optional Discord Role ID to link to this reward.", required=False)
):
    await ctx.defer(ephemeral=True)
    conn = get_db_connection()
    if conn is None:
